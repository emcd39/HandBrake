...
if [ -f "$ARTIFACTS_DIR/gtk/src/ghb" ]; then
    cp "$ARTIFACTS_DIR/gtk/src/ghb" "$PACKAGE_DIR/usr/bin/ghb"
    chmod 755 "$PACKAGE_DIR/usr/bin/ghb"
fi

if [ -f "$ARTIFACTS_DIR/HandBrakeCLI" ]; then
    cp "$ARTIFACTS_DIR/HandBrakeCLI" "$PACKAGE_DIR/usr/bin/HandBrakeCLI"
